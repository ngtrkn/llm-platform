...
class ObjectDetector:
    ...
    def get_model_info(self) -> Dict:
        ...
        # Get input size, handling different model attribute structures
        input_size = 640
        if hasattr(self.model, 'overrides') and isinstance(self.model.overrides, dict):
            input_size = self.model.overrides.get("imgsz", 640)
        elif hasattr(self.model, 'imgsz'):
            input_size = self.model.imgsz
        
        # Convert to native Python type
        if isinstance(input_size, (np.integer, np.floating)):
            input_size = int(input_size)
        elif isinstance(input_size, (list, tuple)) and len(input_size) > 0:
            input_size = int(input_size[0]) if isinstance(input_size[0], (np.integer, np.floating)) else int(input_size[0])
